...
static const std::string spec_args = "(int state_1, int state_2, int pkt_1, int pkt_2,int pkt_3,int pkt_4)";

static const std::string sketch_harness =""
"harness void main" + spec_args + " {\n"
"  StateResult spec_result = codelet(state_1, state_2, pkt_1, pkt_2,pkt_3,pkt_4);\n"
"\n"
"  StateResult impl_result = atom_template(state_1, state_2, pkt_1, pkt_2,pkt_3,pkt_4);\n"
"\n"
"  // Assert values\n"
  ...
  for (const auto * child_decl : dyn_cast<DeclContext>(tu_decl)->decls()) {
    ...
    if (isa<FunctionDecl>(child_decl) and
      ...
      if (ret != 0) {
        ...
        success_count++;
        std::ofstream out("/tmp/success"+ std::to_string(success_count) +".sk");
        out << sketch_contents;
        out.close();
        std::cout << "Sketch succeeded with random seed " << rand_seed << " , input sketch available at /tmp/success"+ std::to_string(success_count) +".sk" << std::endl;
      }
    }
  }
