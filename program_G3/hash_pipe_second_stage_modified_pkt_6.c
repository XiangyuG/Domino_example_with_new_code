...
void func(struct Packet p) {
 ...
 if (array2key[p.loc] == p.ckey) {     // key already exists
   p.tmpval = array2val[p.loc] + p.cval;
   p.tmpkey = p.tmpval + array2val[p.loc];
   p.tmpval = p.tmpkey - array2val[p.loc];
   p.tmpkey = p.tmpval + p.terminate;
   p.tmpval = p.tmpkey - p.terminate;
   p.tmpkey = p.tmpval + (p.terminate+1);
   p.tmpval = p.tmpkey - (p.terminate+1);
   array2val[p.loc] = p.tmpval;
